...
| `JELLYFIN_WATCH_STATE_RETENTION_DAYS` | Watch State Retention Days | number  | `365`      | Watch progress, played flags and favourites untouched for longer than this are deleted by the daily prune task.                                                                                                                                                                                                                                                                                                                 |
...
| Environment Variable                  | UI Setting                | Type    | Default | Description                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------- | ------------------------- | ------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WATCH_STATE_HANDOFF_ENABLED`         | Handoff Enabled           | boolean | `false` | Sends playback events (started, stopped with a watched decision, marked played or unplayed) to configured addons that declare the `playback` resource, so a tracker addon can scrobble what was actually watched instead of guessing from a subtitle request. Only Jellyfin clients produce these events; playback in Stremio reports nothing. |
| `WATCH_STATE_ALLOW_PRIVATE_URLS`      | Allow Private Urls        | boolean | `false` | Allow playback events to be sent to an addon on a private or loopback address, such as `http://tracker:7000` on a Docker network. This lets anyone who can create a configuration make this server send requests to your internal network, so only enable it on a trusted, non-public instance.                                                |
| `WATCH_STATE_MAX_SINKS`               | Max Sinks                 | number  | `3`     | How many addons one configuration may report playback to. A fan-out cap, not a permission: every addon that declares the resource is eligible, this bounds how many requests one play can turn into. 0 disables reporting.                                                                                                                     |
