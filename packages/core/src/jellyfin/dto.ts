...
import { encodeItemId, genreId, personId, viewId } from './ids.js';
import { imageTagsFor, rememberImages } from './images.js';
import { TICKS_PER_MS } from './media.js';
import type {
  ContentDescriptor,
  ItemImages,
  JellyfinDescriptor,
...
export function buildEpisode(
  ...
  return {
    ...
    Path: `/aiostreams/${meta.type}/${meta.id}/${group.name}/${title}${PLAYABLE_EXT}`,
    _aio: { descriptor },
  };
}
...
export function buildBoxSetChild(
  ...
  return {
    ...
    UserData: userDataFromRow(id, playstate, runtimeMs),
    Path: `/aiostreams/${meta.type}/${video.id}/${title}${PLAYABLE_EXT}`,
    _aio: { descriptor },
  };
}
