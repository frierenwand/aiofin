...
async function handleItems(
  ...
  if ((wantsFavorites || wantsPlayed || wantsResumable) && !parentId) {
    ...
    const rows = wantsFavorites
      ? await provider.listFavorites(ctx.uuid, kinds)
      : wantsResumable
        ...
        : await provider.listPlayed(ctx.uuid, kinds);
...
router.get(
  ...
  jf(async (req, res, ctx) => {
    ...
    if (seriesId) {
      ...
      const recent = await provider.listRecentSeries(ctx.uuid, limit * 2);
